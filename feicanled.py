...
class FeicanledInstance:
    ...
    async def set_color(self, rgb: Tuple[int, int, int]):
        r, g, b = rgb
        await self._write([0x7E, 0x07, 0x05, 0x03, r, g, b, 0xEF])
    
    async def set_white(self, intensity: int):
        await self._write([0x7E, 0x07, 0x05, 0x03, 0, 0, 0, 0xEF])

    async def turn_on(self):
        await self._write(bytearray([0x7E, 0x04, 0x04, 0x01, 0xFF, 0xFF, 0xFF, 0xEF]))
        
    async def turn_off(self):
        await self._write(bytearray([0x7E, 0x04, 0x04, 0xFF, 0xFF, 0xFF, 0xFF, 0xEF]))
